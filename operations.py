...
class TaxiiClient(object):
    ...
    def make_request_taxii(self, endpoint=None, method='GET', data=None, params=None, files=None):
        try:
            ...
            headers = {'Authorization': "Basic " + b64_credential,
                       'Accept': 'application/vnd.oasis.taxii+json', 'Content-Type': 'application/json'}
    ...
    def make_request_stix(self, endpoint=None, method='GET', data=None, params=None, files=None):
        try:
            url = self.server_url + 'taxii2/' + endpoint
            b64_credential = base64.b64encode((self.api_key + ":" + self.api_password).encode('utf-8')).decode()
            headers = {'Authorization': "Basic " + b64_credential,
